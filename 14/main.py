...
def draw_path(cave_system, ori, dest, offset=0):
    ori_x = ori[0]+offset
    ori_y = ori[1]

    dest_x = dest[0]+offset
    dest_y = dest[1]
...
def part_one(input_file):
    with open(input_file) as f:
        cave_system = []
        sand_entry_point = [500,0] # CHANGE LATER 500,0
        ...
        while(True):
            try:
                sand_fall(cave_system, sand_entry_point)
                resting_sand += 1
            except:
                break
...
def part_two(input_file):
    with open(input_file) as f:
        cave_system = []
        sand_entry_point = [6+7,0] # CHANGE LATER 500,0
        ...
        for column in range(max_columns+1):
            cave_system[len(cave_system)-1][column] = '#'
        ...
        for line in lines:
            ...
            for i, coord in enumerate(path_coordinates):
                cave_system[coord[1]][coord[0]+7] = '#'
                if (i > 0):
                    prev = path_coordinates[i-1]
                    cave_system = draw_path(cave_system, prev, coord, 7)
